...
*/
	...
	//	Utility functions for this layer
		...
		function queue_post_voters_flaggers($postid)
		{
			...
			if (is_array($posts))
				foreach ($posts as $post) {
					if (isset($post['raw']['opostid']))
						$this->queue_post_voters_flaggers($post['raw']['opostid']); // can be relevant for flags
					if (isset($post['raw']['postid']))
						$this->queue_post_voters_flaggers($post['raw']['postid']);
				}
		}
		...
		function get_post_voters_flaggers($postid)
		{
			require_once QA_INCLUDE_DIR.'qa-util-sort.php';
			
			if (!isset($this->qa_voters_flaggers_cache[$postid])) {
				$this->queue_post_voters_flaggers($postid);
		...
		function main()
		{
			foreach ($this->content as $key => $part) {
				...
				elseif (strpos($key, 'q_view')===0) {
					$this->queue_post_voters_flaggers($part['raw']['postid']);
	...
	//	Actual output of the voters and flaggers
		...
		function post_meta_flags($post, $class)
		{
			...
			if (isset($postid)) {
				$votersflaggers=$this->get_post_voters_flaggers($postid);
