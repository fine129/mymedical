...
//	Find the user profile and questions and answers for this handle
	...
	@list($useraccount, $userprofile, $userfields, $usermessages, $userpoints, $userrank, $favorite)=
...
//	Check the user exists and work out what can and can't be set (if not using single sign-on)
	...
	if (!QA_FINAL_EXTERNAL_USERS) { // if we're using integrated user management, we can know and show more
		...
		$wallposterrorhtml=qa_wall_error_html($loginuserid, $useraccount, false);
	...
	if (!QA_FINAL_EXTERNAL_USERS) {
		...
		if (qa_clicked('dowallpost')) {
			...
			elseif (!qa_check_form_security_code('wall-'.$useraccount['handle'], qa_post_text('code')))
				...
				$wallposterrorhtml=qa_wall_error_html($loginuserid, $useraccount, true); // re-check with limits this time
		...
		if ($usereditbutton) {
			...
			if ($userediting) {
				...
				if (isset($maxlevelassign)) {
					...
					$leveloptions=array(QA_USER_LEVEL_BASIC);
					if (qa_opt('moderate_users'))
						$leveloptions[]=QA_USER_LEVEL_APPROVED;
						
					array_push($leveloptions, QA_USER_LEVEL_EXPERT, QA_USER_LEVEL_EDITOR,
						QA_USER_LEVEL_MODERATOR, QA_USER_LEVEL_ADMIN, QA_USER_LEVEL_SUPER);
